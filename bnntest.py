...
class MLPLayer(nn.Module):
    ...
    def get_random(self):
        return Variable(torch.Tensor(self.n_input, self.n_output).normal_(0, self.sigma_prior).to(device)), Variable(torch.Tensor(self.n_output).normal_(0, self.sigma_prior).to(device))
...
# Initialize network
net = MLP(n_input, sigma_prior).to(device)
# net = net.cuda()
